...
In `lavaan`, models are described with a single block of syntax. Regressions are defined with the tilde `~`, variances and covariances with a double tilde `~~`, and intercepts with `~ 1`. To define the latent variables, we use the special operator `=~`, which can be read as <em>measured</em> <em>by</em>.
...
1. If necessary, install `lavaan`. Then, load the package and dataset: 
```{r, eval=F}
...
MI_LCS.fit <- lavaan(MI_LCS.model, data=dfwide, meanstructure=T, int.ov.free=T)
...
# Measurement error variances and covariances
	...
	X1 ~~ label("covMer")*Y1
 	...
 	X4 ~~ label("covMer")*Y4
 	X5 ~~ label("covMer")*Y5
# In a LCS model, the means of the variables  are defined by the latent level at t=0 and the additive component. 
#However, lavaan automatically estimates the intercepts of the observed variables across all time points. 
#To avoid this redundancy, we must fix the intercepts of the observed variables to 0:
	 X1 + X2 + X3 + X4 + X5 ~ 0*1 
	 Y1 + Y2 + Y3 + Y4 + Y5 ~ 0*1 
...
4. Estimate the model parameters:
``` {r, eval=TRUE, results='hide', echo=T, message=F}
BLCS.fit <- lavaan(BLCS.model, data=dfwide, meanstructure=T, int.ov.free=T)
...
### Measurement structure
# Factor loadings: Configural and weak invariance
	lx1 =~ label("lambda_X1")*X1_t1 + label("lambda_X2")*X2_t1 
	lx2 =~ label("lambda_X1")*X1_t2 + label("lambda_X2")*X2_t2 
	lx3 =~ label("lambda_X1")*X1_t3 + label("lambda_X2")*X2_t3 
	lx4 =~ label("lambda_X1")*X1_t4 + label("lambda_X2")*X2_t4 
	lx5 =~ label("lambda_X1")*X1_t5 + label("lambda_X2")*X2_t5 
	ly1 =~ label("lambda_Y1")*Y1_t1 + label("lambda_Y2")*Y2_t1 + label("lambda_Y3")*Y3_t1 
	ly2 =~ label("lambda_Y1")*Y1_t2 + label("lambda_Y2")*Y2_t2 + label("lambda_Y3")*Y3_t2 
	ly3 =~ label("lambda_Y1")*Y1_t3 + label("lambda_Y2")*Y2_t3 + label("lambda_Y3")*Y3_t3 
	ly4 =~ label("lambda_Y1")*Y1_t4 + label("lambda_Y2")*Y2_t4 + label("lambda_Y3")*Y3_t4 
	ly5 =~ label("lambda_Y1")*Y1_t5 + label("lambda_Y2")*Y2_t5 + label("lambda_Y3")*Y3_t5 
# Intercepts of the indicators: Strong invariance
...
4. Estimate the model parameters:
``` {r, eval=TRUE, results='hide', echo=T, message=F}
MI_BLCS.fit <- lavaan(MI_BLCS.model, data=dfwide, meanstructure=T, int.ov.free=T)
