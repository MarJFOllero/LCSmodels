...
# Measurement error variances and covariances
	...
	X1 ~~ label("covMer")*Y1
 	...
 	X4 ~~ label("covMer")*Y4
 	X5 ~~ label("covMer")*Y5
# In a LCS model, the means of the variables over time are defined by the latent level at t=0 and the additive component. However, lavaan automatically estimates the intercepts of the observed variables across all time points. To avoid this redundancy, we must fix the intercepts of the observed variables to 0:
	 X1 + X2 + X3 + X4 + X5 ~ 0*1 
	 Y1 + Y2 + Y3 + Y4 + Y5 ~ 0*1 
