 ...
 * Choose a name for the model. By setting the argument `type` to *RAM* (Reticular Action Model), the model is defined in terms of single-headed and double-headed paths between variables, just as in a path diagram. 
 * `manifestVars` and `latentVars` are used to specify which variables are manifest and which are latent. 
 * `mxData()` allows to select the dataset as input and indicate that it contains raw scores by setting the argument type to *raw* (it is also possible to use covariance or correlation matrices as inputs). 
