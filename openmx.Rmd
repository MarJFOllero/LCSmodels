...
In `OpenMx`, the first step is to define the names of all the latent and manifest variables. The names of the manifest variables in the model must match those in the dataset.

Before building the model, some preparations are necessary:    

 * Choose a name for the model. By setting the argument `type` to <strong>RAM</strong> (Reticular Action Model), the model is defined in terms of single-headed and double-headed paths between variables, just as in a path diagram. 
 * `manifestVars` and `latentVars` are used to specify which variables are manifest and which are latent. 
 * `mxData()` allows to select the dataset as input and indicate that it contains raw scores by setting the argument type to <strong>raw</strong> (it is also possible to use covariance or correlation matrices as inputs). 
  
  
Once we have finished the preparations, building the model is very intuitive. All paths are defined through the function `mxPath()`.  `from` and `to` are used to indicate the origins and endings of the paths, `arrows` to indicate regression (`=1`) or covariance (`=2`), `free` to fix (`=FALSE`) or freely estimate (`=TRUE`) the parameters defining the path, `values` to indicate the value of the fixed parameter when `free=FALSE` or to set a value from which to initialize the estimation process when `free=TRUE`, and `labels` to name the parameter defining the paths. Using the same label for multiple paths implies imposing an equality constraint, meaning that a single parameter will be estimated across those paths.  
  
  
An important advantage of `OpenMx` is that it does not require to define each single path separately. For example, it is possible to define all the paths from $y(t-1)$ to $y(t)$ just by specifying `from=latent_y[1:(t-1)], to=latent_y[2:t]` inside `mxPath()`.
...
1. If necessary, install `OpenMx`. Then, load the package and dataset: 
```{r, eval=F}
...
2. Define the variables of the model:
```{r}
...
LCS.model <- mxModel("Univariate LCS", type = "RAM",
    ...
    ### LATENT STRUCTURE
      ...
      # “one” is used to define intercepts. In this case, the path from “one” to y0 and ya represents the means of these latent variables.
...
## Stochastic extension of the univariate LCS model
It is possible to include the innovation variance by just adding a new `mxPath()` to the already built LCS.model:
...
# Run the new model including innovation variance
...
2. Define the number of measurement occasions:
```{r}
Tmax <- 5 # Number of measurement occasions
...
# Create the labels for the latent state at t=0, the additive component, the manifest scores, 
#their corresponding latent scores, and the latent changes:
x0 <- "x0"
xa <- "xa"
y0 <- "y0"
ya <- "ya"
...
## Stochastic extension of the bivariate LCS model

It is possible to include the innovation variance by just adding a new `mxPath()` to the already built BLCS.model:
``` {r, eval=TRUE, results='hide', echo=T, message=FALSE}
...
# Run the new model including innovations variances and covariance
...
MI_BLCS.model <- mxModel("Bivariate LCS", type="RAM",
    ...
    ### LATENT STRUCTURE
      ...
      ## MEASUREMENT STRUCTURE   
              ...
              # mxPath(from=latent_x, to=indic_X3, arrows=1, free=TRUE, values=0, labels="lambda_X3"),    
              mxPath(from=latent_y, to=indic_Y1, arrows=1, free=FALSE, values=1, labels="lambda_Y1"),
              mxPath(from=latent_y, to=indic_Y2, arrows=1, free=TRUE, values=0, labels="lambda_Y2"),
              mxPath(from=latent_y, to=indic_Y3, arrows=1, free=TRUE, values=0, labels="lambda_Y3"),
              ...
              # mxPath(from="one", to=indic_X3, arrows=1, free=TRUE, values=0, labels="tau_X3"),
              mxPath(from="one", to=indic_Y1, arrows=1, free=TRUE, values=0, labels="tau_Y1"),
              mxPath(from="one", to=indic_Y2, arrows=1, free=TRUE, values=0, labels="tau_Y2"),
              mxPath(from="one", to=indic_Y3, arrows=1, free=TRUE, values=0, labels="tau_Y3"),
              ...
              # mxPath(from=indic_X3, arrows=2, free=TRUE, values=0, labels="mer_X3"),                      
              mxPath(from=indic_Y1, arrows=2, free=TRUE, values=0, labels="mer_Y1"),
              mxPath(from=indic_Y2, arrows=2, free=TRUE, values=0, labels="mer_Y2"),
              mxPath(from=indic_Y3, arrows=2, free=TRUE, values=0, labels="mer_Y3")
            # mxPath(from=indic_X1, to=indic_Y1, arrows=2, free=TRUE, values=.5, labels="covMer1"),
            # mxPath(from=indic_X2, to=indic_Y2, arrows=2, free=TRUE, values=.5, labels="covMer2"),
            # mxPath(from=indic_X3, to=indic_Y3, arrows=2, free=TRUE, values=.5, labels="covMer3")
            )
